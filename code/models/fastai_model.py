...
class fastai_model(ClassificationModel):
    ...
    def fit(self, X_train, y_train, X_val, y_val):
        ...
        if(self.pretrainedfolder is None): #from scratch
            ...
            learn.data.valid_dl.num_workers = 0  
    ...
    def _get_learner(self, X_train,y_train,X_val,y_val,num_classes=None):
        ...
        # needed for gpu support, moves data from cpu.
        if isinstance(X_train, torch.Tensor):
            X_train = [x.cpu().numpy() for x in X_train]
        if isinstance(X_val, torch.Tensor):
            X_val = [x.cpu().numpy() for x in X_val]
        if isinstance(y_train, torch.Tensor):
            y_train = [y.cpu().numpy() for y in y_train]
        if isinstance(y_val, torch.Tensor):
            y_val = [y.cpu().numpy() for y in y_val]
