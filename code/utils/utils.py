...
def compute_label_aggregations(df, folder, ctype):
    ...
    # added a new type superdiagnosticNORMandMI and superdiagnosticNotNORMandMI 
    if ctype in ['diagnostic', 'subdiagnostic', 'superdiagnostic', 'superdiagnosticNORMandMI', 'superdiagnosticNotNORMandMI']:
            ...
            for key in y_dic.keys():
                if key in diag_agg_df.index:
                    ...
                    if str(c) != 'nan':
                        tmp.append(c)
            return list(set(tmp))
        # added new function for my case    
        def aggregate_NORMandMI_diagnostic(y_dic):
            tmp = []
            has_other = False
            for key in y_dic.keys():
                if key in diag_agg_df.index:
                    c = diag_agg_df.loc[key].diagnostic_class
                    # Only keep NORM and MI, while check for nan
                    if str(c) != 'nan':
                        if c in ['STTC', 'CD', 'HYP']:
                            has_other = True
                        elif c in ['NORM', 'MI']:
            ...
            if has_other:
                return []
            return list(set(tmp))
        def aggregate_NotNORMandMI_diagnostic(y_dic):
            tmp = []
            has_norm_mi = False
            for key in y_dic.keys():
                if key in diag_agg_df.index:
        ...
        # added a new option for my case of only MI and Norm    
        elif ctype == 'superdiagnosticNORMandMI':
            df['superdiagnosticNORMandMI'] = df.scp_codes.apply(aggregate_NORMandMI_diagnostic)
            df['superdiagnosticNORMandMI_len'] = df.superdiagnosticNORMandMI.apply(lambda x: len(x))
            
        elif ctype == 'superdiagnosticNotNORMandMI':
            df['superdiagnosticNotNORMandMI'] = df.scp_codes.apply(aggregate_NotNORMandMI_diagnostic)
            df['superdiagnosticNotNORMandMI_len'] = df.superdiagnosticNotNORMandMI.apply(lambda x: len(x))
...
def select_data(XX,YY, ctype, min_samples, outputfolder):
    ...
    if ctype == 'diagnostic':
        ...
        counts = pd.Series(np.concatenate(YY.superdiagnosticNORMandMI.values)).value_counts()
        counts = counts[counts > min_samples]
        YY.superdiagnosticNORMandMI = YY.superdiagnosticNORMandMI.apply(lambda x: list(set(x).intersection(set(counts.index.values))))
        YY['superdiagnosticNORMandMI_len'] = YY.superdiagnosticNORMandMI.apply(lambda x: len(x))
        X = XX[YY.superdiagnosticNORMandMI_len > 0]
        Y = YY[YY.superdiagnosticNORMandMI_len > 0]
        mlb.fit(Y.superdiagnosticNORMandMI.values)
        y = mlb.transform(Y.superdiagnosticNORMandMI.values)
        
    elif ctype == 'superdiagnosticNotNORMandMI':
        counts = pd.Series(np.concatenate(YY.superdiagnosticNotNORMandMI.values)).value_counts()
        counts = counts[counts > min_samples]
        YY.superdiagnosticNotNORMandMI = YY.superdiagnosticNotNORMandMI.apply(lambda x: list(set(x).intersection(set(counts.index.values))))
        YY['superdiagnosticNotNORMandMI_len'] = YY.superdiagnosticNotNORMandMI.apply(lambda x: len(x))
        X = XX[YY.superdiagnosticNotNORMandMI_len > 0]
        Y = YY[YY.superdiagnosticNotNORMandMI_len > 0]
        mlb.fit(Y.superdiagnosticNotNORMandMI.values)
        y = mlb.transform(Y.superdiagnosticNotNORMandMI.values)   
...
def generate_ptbxl_summary_table(selection=None, folder='../output/'):
    ...
    # get models, updated for windows path.
    models = {}
    for i, exp in enumerate(exps):
        if selection is None:
            model_paths = glob.glob(os.path.join(folder, str(exp), 'models', '*'))
            exp_models = [os.path.basename(m) for m in model_paths]
        else:
